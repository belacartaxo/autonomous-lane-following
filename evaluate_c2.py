"""
evaluate_c2.py — Avaliação da condição C2 (Sensor Noise) para DQN e PPO.

Uso:
    python evaluate_c2.py --algo ppo --model ./models/ppo_noise/ppo_best
    python evaluate_c2.py --algo dqn --model ./models/dqn_noise/dqn_best
...
def run(algo: str, model_path: str):
    print(f"\n{'=' * 60}")
    print("  C2 Sensor Noise Evaluation")
    ...
    out_path = f"./results/c2_{algo.lower()}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Avaliação C2 Sensor Noise")
    parser.add_argument(
        "--algo",
        required=True,
        choices=["dqn", "ppo"],
