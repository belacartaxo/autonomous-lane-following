...
class WebotsCriticalVehicleEnv(WebotsVehicleEnv):
    ...
    def __init__(self):
        ...
        # One-time bonus given the moment the obstacle disappears
    ...
    def _compute_reward(self, obs, action):
        ...
        if dynamic_obstacle_active:
            # When the obstacle activates again, reset the resume bonus
            self.resume_reward_given = False
        ...
        # Non-critical obstacle phase 

        self.not_stopped_step_count = 0

        # Detect transition: obstacle just disappeared
        just_cleared = self.was_obstacle_active and not dynamic_obstacle_active
        if just_cleared and not self.resume_reward_given:
            print("Obstacle passed — resume bonus given to the agent.")
            # This bonus is given only once at the moment of transition
            # to break the learned stopping inertia
        ...
        # Apply resume bonus (one-time, on the first step after the obstacle)
        reward += resume_bonus

        # When there is no critical obstacle, the agent must move forward.
        # Strong penalties to break the habit of staying still.
        if vehicle_movement < self.inactive_min_movement:
            reward -= self.inactive_low_movement_penalty

        # Penalty for weak throttle when there is no reason to stop.
        if throttle <= self.forward_throttle_threshold:
            reward -= self.inactive_no_throttle_penalty

        # Reward positive throttle to encourage lane resumption.
        if throttle > self.forward_throttle_threshold:
            reward += throttle * self.inactive_forward_bonus_weight

        # Penalty for reversing outside of critical obstacle situations.
        if throttle < -self.forward_throttle_threshold:
